...
const changePassword = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    console.log('User from request:', user ? `ID: ${user._id}, Email: ${user.email}` : 'No user in request');
    ...
    // Compare as strings to ensure proper comparison
    console.log(`Comparing requestedId: ${userId} with userTokenId: ${user._id.toString()}`);
    if (userId !== user._id.toString()) {
        return sendResponse(res, {
            statusCode: httpStatus.FORBIDDEN,
            status: false,
