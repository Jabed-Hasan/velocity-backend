...
router.post('/', auth(USER_ROLE.admin), (req: Request, res: Response, next: NextFunction) => {
  console.log('Post car endpoint hit with content-type:', req.headers['content-type']);
    ...
    try {
      ...
      if (req.file) {
        ...
        try {
          ...
          const { secure_url } = await sendImageToCloudinary(imageName, imageSource);
      ...
      const validationResult = carValidationSchema.safeParse({ body: req.body });
...
// 5. Update a Car
router.patch('/:id', auth(USER_ROLE.admin), (req: Request, res: Response, next: NextFunction) => {
  console.log('Update car endpoint hit with content-type:', req.headers['content-type']);
  ...
  singleUpload(req, res, async (uploadErr) => {
    ...
    try {
      ...
      const validationResult = carUpdateValidationSchema.safeParse({ body: req.body });
      if (!validationResult.success) {
        console.error('Validation error:', validationResult.error);
        return res.status(400).json({
          success: false,
